//! Configuration management for Grakno.
//!
//! Supports workspace-local `.grakno.toml` files with validation.
...
impl Config {
    /// Load configuration from a `.grakno.toml` file.
    ///
    ...
    pub fn default_with_comments() -> String {
        r#"# Grakno configuration file
# Place this file in your workspace root as `.grakno.toml`
...
mod tests {
    ...
    fn config_load_missing_file() {
        let result = Config::load(Path::new("/nonexistent/.grakno.toml")).unwrap();
