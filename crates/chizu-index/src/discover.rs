...
mod tests {
    ...
    #[test]
    fn discover_chizu_workspace() {
        // Discover the chizu workspace itself
        ...
        // Workspace name comes from directory name, which may be 'chizu'
        // depending on git clone directory name
        assert!(!ws.name.is_empty());
        assert!(ws.crates.len() >= 2);

        let names: Vec<_> = ws.crates.iter().map(|c| c.name.as_str()).collect();
        assert!(names.contains(&"chizu-core"));
        assert!(names.contains(&"chizu-index"));

        // chizu-index depends on chizu-core
        let has_dep = ws
            .deps
            .iter()
            .any(|d| d.from == "chizu-index" && d.to == "chizu-core");
        assert!(has_dep, "expected chizu-index -> chizu-core dep");
    }
}
